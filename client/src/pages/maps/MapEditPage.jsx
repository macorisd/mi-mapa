...
import { useAuth } from "../../context/AuthContext"; // Asegúrate de que este hook existe para verificar autenticación

const EventEditPage = () => {
  const { eventos, media } = useAPI();
  const { getActualEvent, setActualEvent } = useData();
  const { isLogged } = useAuth(); // Hook para comprobar si el usuario está logueado
  ...
  const parseInputToTimestamp = (input) => input;
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        if (eventResponse.status >= 200 && eventResponse.status < 300) {
          ...
          navigate("/events/" + event._id); // Redirige a la página principal
        } else {
